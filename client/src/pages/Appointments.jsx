  ...
  return docInfo && (
    <div>
      <div className='flex flex-col sm:flex-row gap-4'>
        ...
        <div >
          <img className='bg-primary w-ful sm:max-w-72 rounded-lg' src={docInfo.image} alt="" />
        </div>
        ...
        <div className='flex gap-3 items-center w-full overflow-x-scroll mt-4'>
          {
            docSlots.length && docSlots.map((item,index) => (
              <div onClick={() => setSlotIndex(index)} className={`${item[0]? '': 'hidden'} text-center py-6 min-w-16 rounded-full cursor-pointer ${slotIndex === index? 'bg-primary text-white':'border border-gray-200'}`} key={index}>
