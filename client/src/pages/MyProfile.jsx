...
const [userData,setUserData] = useState({
  ...
  email: 'aneeshappid@gmail.com',
  phone: '+1 123 456 7890',
  address: {
    line1:"57th Cross, Netaji Nagar ",
    line2:"Church Road, Banglore"
  },
  gender: 'Male',
  dob:'2000-10-20'
});
...
const [isEdit,setIsEdit] = useState(false);
  ...
  return (
    <div className='max-w-lg flex flex-col gap-2 text-sm'>
      ...
      {
        isEdit
        ? <input className='bg-gray-50 text-3xl font-medium max-w-660 mt-4' type='text' value={userData.name} onChange={e => setUserData(prev => ({...prev,name:e.target.value}))} />
      ...
      <div className='mt-10'>
        {
          ...
          : <button className='border border-primary px-8 py-2 rounded-full hover:bg-primary transition-all hover:text-white duration-200' onClick={() => setIsEdit(true)}>Edit</button>
        }
      </div>
    </div>
  )
